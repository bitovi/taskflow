...
export default async function TasksPage() {
    const [tasksResult, usersResult] = await Promise.all([
        searchTasks({ page: 1, pageSize: 10 }),
        getAllUsers()
    ]);
    
    if (tasksResult.error || usersResult.error) {
        console.error("Error fetching data:", tasksResult.error || usersResult.error)
    ...
    return (
        <div className="flex-1 space-y-4 p-4 md:p-8 pt-6">
            ...
            <Suspense fallback={<div>Loading tasks...</div>}>
                <TasksSearch initialTasks={tasksResult.tasks || []} users={usersResult.users || []} />
            </Suspense>
        </div>
    )
}
