...
5. **Run the tests**: Run the unit and E2E tests to see if the bug can be reproduced in the test environment. Note any failures or unexpected behaviors. The dev server can be run as a VS Code task, as can the tests. Be sure to use tasks instead of running commands directly in the terminal to ensure the environment is set up correctly.
  - The E2E tests expect a local dev server to be running at `http://localhost:3000`, so make sure to kill any other instances running on port 3000 before running the dev server (this should be built into the task).
